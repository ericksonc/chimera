...
class RAGWidget(Widget[RAGConfig]):
    ...
    **Context Expansion (include_neighboring_chunks=True)**:
    ...
    - Format: [Context: chunk N-1] → >> [Matched: chunk N] << → [Context: chunk N+1]
    ...
    async def _initialize_embeddings(self) -> None:
        ...
        registry_hits = {}  # file_path → RegistryEntry
        ...
        if files_same_collection_reuse:
            print(
                f"✓ {len(files_same_collection_reuse)}/{total_files} files unchanged in collection (reusing embeddings)"
            )
        if files_cross_collection_reuse:
            print(
                f"♻️  {len(files_cross_collection_reuse)}/{total_files} files reused from other collections (global registry)"
            )
        if files_to_embed:
        ...
        for file_path in files_cross_collection_reuse:
            ...
            try:
                ...
                print(
                    f"  ♻️  Copied {copied_count} chunks for {file_path} from {entry.collection_name}"
                )
                successful_registry_reuse.append(file_path)
            except ValueError as e:
                # If copy fails (source not found, chunks missing), fall back to embedding
                print(f"  ⚠️ Failed to copy chunks for {file_path} (ValueError: {e})")
                print("  → Will embed this file instead")
                files_to_embed.append(file_path)
            except Exception as e:
                # Unexpected error - log but re-raise
                print(
                    f"  ⚠️ Unexpected error copying chunks for {file_path} ({type(e).__name__}: {e})"
                )
                raise
        ...
        print(f"  → Total: {len(all_chunks)} chunks across {len(all_file_chunks)} files to embed")
        ...
        for i, chunk in enumerate(all_chunks):
            ...
            if current_batch and (would_exceed_items or would_exceed_tokens):
                batch_num += 1
                print(
                    f"  → Batch {batch_num}: {len(current_batch)} chunks ({current_batch_tokens:,} tokens)..."
                )
        ...
        if current_batch:
            batch_num += 1
            print(
                f"  → Final batch {batch_num}: {len(current_batch)} chunks ({current_batch_tokens:,} tokens)..."
            )
            result = await embedding_service.embed_texts(current_batch, input_type="document")
            all_embeddings.extend(result.embeddings)
        ...
        print(f"✓ RAG embeddings ready ({len(self.documents)} files)")
        if registry_reuse_count > 0:
            print(
                f"  → Reused {reused_chunks_from_registry} chunks from global registry (saved API calls!)"
            )
        if same_collection_reuse_count > 0:
            print(f"  → {same_collection_reuse_count} files unchanged in this collection")
