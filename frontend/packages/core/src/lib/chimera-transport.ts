...
type UserInputDeferredTools = {
  kind: "deferred_tools";
  approvals: Record<string, boolean | { approved: boolean; message?: string; override_args?: any }>;
...
export class ChimeraTransport implements ChatTransport<UIMessage> {
  ...
  async sendMessages(options: {
    ...
    if (this.pendingToolApprovals) {
      ...
      console.log("[ChimeraTransport] Using pending tool approvals:", this.pendingToolApprovals);
      userInput = {
        kind: "deferred_tools",
    ...
    console.log("[ChimeraTransport] First 2 thread_protocol events:",
    ...
    return new ReadableStream<UIMessageChunk>({
      ...
      pull: async (controller) => {
        try {
          ...
          console.log('[ChimeraTransport] Received chunk, bytes:', decoded.length);
          buffer += decoded;
          ...
          for (const line of lines) {
            ...
            if (line.startsWith("data: ")) {
              ...
              try {
                ...
                console.log('[ChimeraTransport] Raw VSP event:', event.type, event.toolName || '');
   ...
   */
  private handleVSPEvent(event: any): UIMessageChunk | null {
    ...
    if (eventType === "start" || eventType === "finish" || eventType === "abort") {
  ...
  // Text accumulation
  private startTextAccumulator(event: any) {
  ...
  private startToolCallAccumulator(event: any) {
  ...
  private startReasoningAccumulator(event: any) {
    const id = event.id || event.messageId;
    this.accumulators.set(id, {
      id,
      type: "reasoning",
  ...
  private async persistEvents() {
    ...
    console.log('[ChimeraTransport] Event types:', this.pendingEvents.map(e => e.type).join(', '));

    try {
      // Append to storage via adapter
      await this.storageAdapter.appendThreadEvents(this.threadId, this.pendingEvents);
   ...
   */
  private async persistToolCallImmediate(event: ThreadProtocolEvent): Promise<void> {
    try {
      console.log(`[ChimeraTransport] Immediately persisting tool call: ${(event as any).toolName}`);
      ...
      console.log(`[ChimeraTransport] ✓ Tool call persisted: ${(event as any).toolCallId}`);
    } catch (error) {
  ...
  async loadHistory(): Promise<void> {
    try {
      ...
      console.log("[ChimeraTransport] First event:", JSON.stringify(events[0], null, 2));
